...
The active implementation unit is Phase 12.3: expose one direct digest-pinned Python base stage, enforce the approved Python series, Debian variant, digest, and stage graph with a strict dependency-free validator, retain deterministic base-policy evidence, and let Dependabot propose bounded weekly patch refreshes for review without automatic merging or new authority.

Later Phase 12 units will add locked and hashed Python dependencies, vulnerability policy, provenance attestations, image signing, persistent services, workload identity, secret delivery, observability, backup and restore, deployment testing, and container endurance certification. The public web product remains explicitly deferred. Containerization and evidence generation must not bypass source validation, tool policy, approval, token verification, canonical state stores, recovery authority, or evaluation authority.
