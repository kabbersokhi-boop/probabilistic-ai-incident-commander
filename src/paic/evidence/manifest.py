"""Manifest models for exported operational evidence artifacts."""
...
class EvidenceColumnManifest(StrictModel):
    name: NonEmptyText
    dtype: NonEmptyText
    nullable: bool


class EvidenceTableManifest(StrictModel):
    name: NonEmptyText
    ...
    columns: list[EvidenceColumnManifest]
    ...
    def validate_relative_path(cls, value: str) -> str:
        ...
        if path.is_absolute() or ".." in path.parts or not path.parts:
            raise ValueError("relative_path must stay inside the evidence directory")
        return value

    @model_validator(mode="after")
    def validate_metadata(self) -> EvidenceTableManifest:
...
class EvidenceRuntimeManifest(StrictModel):
...
class EvidenceManifest(StrictModel):
    ...
    runtime: EvidenceRuntimeManifest
    source_dataset_id: NonEmptyText
    source_dataset_manifest_sha256: Annotated[str, Field(pattern=r"^[a-f0-9]{64}$")]
    source_analytics_manifest_sha256: Annotated[str, Field(pattern=r"^[a-f0-9]{64}$")] | None = None
    source_detection_manifest_sha256: Annotated[str, Field(pattern=r"^[a-f0-9]{64}$")] | None = None
    source_impact_manifest_sha256: Annotated[str, Field(pattern=r"^[a-f0-9]{64}$")] | None = None
    evidence_config_sha256: Annotated[str, Field(pattern=r"^[a-f0-9]{64}$")]
    logical_start_at: datetime
    logical_end_at: datetime
    evidence_record_count: Annotated[int, Field(ge=0)]
    timeline_event_count: Annotated[int, Field(ge=0)]
    lineage_node_count: Annotated[int, Field(ge=0)]
    lineage_edge_count: Annotated[int, Field(ge=0)]
    quality_error_count: Annotated[int, Field(ge=0)]
    tables: list[EvidenceTableManifest]
