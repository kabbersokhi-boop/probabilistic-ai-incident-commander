...
def test_concurrent_readers_see_only_complete_generations(tmp_path: Path) -> None:
    ...
    try:
        for _ in range(20):
            publisher = AtomicDirectoryPublisher(target, overwrite=True)
            with publisher as staging:
                (staging / "value.txt").write_text("generation-1", encoding="utf-8")
                publisher.commit()
            inspections += 1
    finally:
        stop.set()
        thread.join(timeout=5)
    assert inspections == 20
    assert observations == []


def test_exports_publish_valid_complete_dataset(tmp_path: Path) -> None:
    output = tmp_path / "dataset"
    result = SimulationResult(seed=1, events=[], spans=[])
    export_dataset(result, output, overwrite=True)
...
def test_publication_generations_are_monotonic(tmp_path: Path) -> None:
    target = tmp_path / "artifact"
