...
export type Bundle = { schema_version: string; bundle_kind: string; disclaimer: string; workspace_id: string; display_name: string; stages: {key:string;title:string;status:string;summary:string;details:string[]}[]; files: PublicFile[]; presentation?: Record<string, unknown> };
